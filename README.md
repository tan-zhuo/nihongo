...
- **文章打字练习**：52 篇 N5–N1 分级文章（40 篇原创短文 + 12 篇日本经典昔话：桃太郎、浦島太郎、かぐや姫等），一行原文一行输入逐字实时比对，汉字带假名注音（可开关），IME 变换未确认的假名不计错。发现打错了可以退回上一行改：把当前行删空后再按一次退格键（手机上点行尾的「改上一行」）。完成后显示正确率、速度（字/分钟）、用时、错误次数。
